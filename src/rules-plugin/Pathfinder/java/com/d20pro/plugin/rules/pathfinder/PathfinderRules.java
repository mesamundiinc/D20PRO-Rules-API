...
public class PathfinderRules extends AbstractRules
{
  public PathfinderRules()
  {
    super();
  }
...
//      byte[] abilities = creature.getAbilities();
    ...
    public void calculateAbilityScore(CreatureTemplate template, byte id)
    {
      ...
      if (base == 127) {
        ...
        if (base == 0 && template.getAbilityScore(id, false) != 0) {
//          System.out.println(template.getAbilityScore(id));
          template.setAbilityScoreBase(id, template.getAbilityScore(id), false);
        }
      }
    ...
    public void calculateAbilityScores(CreatureTemplate template)
    {
      ...
      {
        if (i == CreatureTemplate.NO_ABILITY || i == 127)
          continue;
...
//        template.isHistorical(false);
      ...
      byte[] abilities = template.getAbilitiesBase();
      int count = 0;
      for(int i = 0; i < abilities.length; i++) {
        if (abilities[i] == 10) count++;
      }
...
//      if( template.isHistorical() )
      if (count == abilities.length)
      {
        ...
        for(byte i = 0; i < abilities.length; i++ )
        {
          ...
          if ((abilities[i] == Ability.NONE) || (abilities[i] == 127))
...
//        byte[] baseSaves = template.getSavesBase();
        for(byte i = 0; i < saves.length; i++)
        {
          template.setSaveBase(i, (byte) 0, false);
        }
